...
{
    public partial class AddBaseKontakteComponent : ComponentBase
    {
        ...
        {
            basekontakte = new SinDarElaVerwaltung.Models.DbSinDarEla.BaseKontakte(){};
        }

        protected async System.Threading.Tasks.Task Form0Submit(SinDarElaVerwaltung.Models.DbSinDarEla.BaseKontakte args)
        {
            basekontakte.BaseID = int.Parse($"{BaseID}");

            try
            {
                var dbSinDarElaCreateBaseKontakteResult = await DbSinDarEla.CreateBaseKontakte(baseKontakte:basekontakte);
                UriHelper.NavigateTo("bases");
            }
            catch (System.Exception dbSinDarElaCreateBaseKontakteException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new BaseKontakte!" });
            }
        }
