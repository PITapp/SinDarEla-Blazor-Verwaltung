...
{
    public partial class AddInfotexteHtmlComponent : ComponentBase
    {
        ...
        {
            infotextehtml = new SinDarElaVerwaltung.Models.DbSinDarEla.InfotexteHtml(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(SinDarElaVerwaltung.Models.DbSinDarEla.InfotexteHtml args)
        {
            try
            {
                var dbSinDarElaCreateInfotexteHtmlResult = await DbSinDarEla.CreateInfotexteHtml(infotexteHtml:infotextehtml);
                DialogService.Close(infotextehtml);
            }
            catch (System.Exception dbSinDarElaCreateInfotexteHtmlException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new InfotexteHtml!" });
            }
        }
